...
d3.csv("debt.csv", type, function(error, data) {
  ...
  data.forEach(function(d) {
    if (person.has(d.friend)) d.friend = person.get(d.creditor);
    else person.set(d.friend, d.friend = {name: d.friend, index: ++personFriendIndex});
    if (person.has(d.debtor)) d.debtor = person.get(d.debtor);
    else person.set(d.debtor, d.debtor = {name: d.debtor, index: ++personFriendIndex});
    d.debtor.risk = d.risk;
  });

  // Initialize a square matrix of debits and credits.
  for (var i = 0; i <= personFriendIndex; i++) {
    debits[i] = [];
    credits[i] = [];
    for (var j = 0; j <= personFriendIndex; j++) {
      debits[i][j] = 0;
      credits[i][j] = 0;
    }
  }
